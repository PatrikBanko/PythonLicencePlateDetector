...
def detect(img_rgb):
    ...
    new_height = img_rgb.shape[0] / 2
    new_width = img_rgb.shape[1] / 2
    # size = (img_rgb.shape[1] / 2, img_rgb.shape[0] / 2)
    img_resized = cv2.resize(img_resized, (round(new_width), round(new_height)))
    ...
    high_yellow = np.array([172, 111, 255])
...
def recognise(src_path, out_path):
    ...
    # cmd = 'C:/Program' + 'Files/Tesseract-OCR/tesseract.exe '+ src_path + ' ' + out_path + ' -l eng --psm 6 --dpi 300 --oem 1'
    # print(cmd)
    # os.system(cmd)
